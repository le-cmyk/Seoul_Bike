...
def GradientBoosting(request):
    ...
    fig3 = go.Figure(data=fig2.data+ fig1.data , layout=go.Layout(
        title=go.layout.Title(text="Prevision du modele Gradient Boosting (bleu) et valeurs réelles (rouge)")))
...
def BayesianRidge(request):
    ...
    fig2 = px.line(x= x_ax,y= y,title = "a").update_traces(marker=dict(color='yellow'))
    fig3 = go.Figure(data=fig2.data+ fig1.data, layout=go.Layout(
        title=go.layout.Title(text="Prevision du modele Bayesian Ridge (bleu) et valeurs réelles (rouge)") ))
    plot2_html = fig3.to_html(full_html =False , default_height = 500, default_width = 700)
    return plot2_html
...
def RandomForest(request):
    ...
    fig2 = px.line(x= x_ax,y= y,title = "a").update_traces(marker=dict(color='yellow'))
    fig3 = go.Figure(data=fig2.data+ fig1.data, layout=go.Layout(
        title=go.layout.Title(text="Prevision du modele Random Forest (bleu) et valeurs réelles (rouge)")
    ))
    plot2_html = fig3.to_html(full_html =False , default_height = 500, default_width = 700)
    return plot2_html
